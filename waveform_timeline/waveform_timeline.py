...
class WaveformDocker(DockWidget):
    ...
    def _find_timeline_table(self):
        ...
        nativa, pra ler seu offset/zoom (ver TimelineDocker no Krita) e
        manter as colunas da waveform alinhadas com o eixo de frames.
    ...
    def _update_current_frame(self):
        # A barra vermelha reflete o frame real do documento. Durante a
        # reproducao nativa do Krita o currentTime() fica congelado ate
        # o Play parar/pausar (nao ha como ler o frame real em tempo real
        # via scripting), entao ela so atualiza de novo nesse momento -
        # sem tentar estimar/acompanhar o play em andamento. Ao arrastar
        # o cursor na Linha do Tempo nativa ou clicar na waveform o
        # currentTime() muda normalmente e a barra acompanha na hora.
        if self._doc is None:
            return
